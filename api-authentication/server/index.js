...
app.post('/api/auth/sign-in', (req, res, next) => {
  ...
  db.query(sql2, params2)
    .then(result => {
      ...
      // console.log(result);
      // console.log("specific:", result.rows[2]);
      // console.log("specific:", result.rows);
      // console.log("specific:", result.rows[0]);

      // console.log("specific:", result.rows[0].hashedPassword);
      // const selectedHash = result.rows[0].hashedPassword;
      const [user] = result.rows;
      if (!user) {
        throw new ClientError(401, 'invalid login');
      }
      ...
      // argon2.verify(selectedHash, password)
      //   .then(isMatching => {
...
//   .then(hashedPassword => {
//     const hased = hashedPassword;

//     const params2 = [username];
//     const sql2 = `select "userId", "hashedPassword"
// from "users"
// where "username" = $1
// `;
...
// const params = [username, hashedPassword];
// const sql2 = `select "userId", "hasedpassword"
// from "users"
// where "username" = $1
// `;
